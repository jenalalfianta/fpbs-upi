...
export function injectInlineCarousel(raw = '') {
  ...
  return raw.replace(re, (_match, inner) => {
    // Normalisasi baris
    const lines = inner
      .replace(/\r\n?/g, '\n')
      .split('\n')
    ...
    // Ambil opsi JSON di baris pertama (opsional)
    let opts = {};
    let startIdx = 0;
    if (lines[0] && lines[0].startsWith('{') && lines[0].endsWith('}')) {
      try {
        ...
        // abaikan jika JSON invalid
      }
    }

    // Kumpulkan item gambar: "/path.webp|Alt text"
    // Mendukung format satu-baris dipisah spasi: "/a.webp|A /b.webp|B"
    const images = [];
    for (let i = startIdx; i < lines.length; i++) {
      const parts = lines[i].split(/\s+(?=\/)/g); // split pada spasi sebelum '/'
      for (const p of parts) {
        const seg = p.trim();
        if (!seg) continue;
        const [src, ...altParts] = seg.split('|');
    ...
    // ID unik per blok untuk style & script scoped
    const rid = 'crs_' + Math.random().toString(36).slice(2, 9);

    // Slide: jika src kosong → tampilkan teks alt saja
    const slidesHtml = images.map((img, i) => {
      const active = i === 0 ? ' active' : '';
      if (!img.src) {
        return (
`<div class="carousel-slide${active}">
  <div class="flex items-center justify-center w-full h-full p-6 text-center text-gray-600 dark:text-gray-300">${escapeHtml(img.alt)}</div>
</div>`
        );
      }
      return (
`<div class="carousel-slide${active}">
  <img src="${img.src}" alt="${escapeHtml(img.alt)}" class="w-full h-full" loading="lazy" decoding="async" />
    ...
    const dotsHtml = images.map((_img, i) =>
      `<button class="w-3 h-3 rounded-full ${i === 0 ? 'bg-white' : 'bg-gray-400/70'}" data-dot="${i}"></button>`
    ).join('\n');

    // Penting: baris pertama HARUS mulai dengan '<' (tanpa spasi) agar tidak dianggap code block oleh Markdown
    const html =
...
<script>(function(){
  ...
  show(0); start();
    ...
    // Tambahkan newline di depan & belakang supaya aman secara Markdown,
    // dan PASTIKAN baris pertama html dimulai '<' (sudah).
    return '\n' + html + '\n';
  });
}
