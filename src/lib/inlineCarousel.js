...
export function injectInlineCarousel(raw = '') {
  ...
  // Regex tangkap blok :::carousel ... ::: (multiline)
  ...
  return raw.replace(re, (_, inner) => {
    ...
    const dotsHtml = images
      .map(
        (_img, i) =>
          `<button class="w-3 h-3 rounded-full ${i === 0 ? 'bg-white' : 'bg-gray-400/70'}" data-dot="${i}" aria-label="Go to slide ${i + 1}"></button>`
      )
      .join('\n');

    return `
<div id="${rid}" class="relative w-full carousel-wrapper overflow-hidden bg-gray-50 dark:bg-gray-800 m-0 p-0 z-0"
  ...
  <style>
    ...
    #${rid} img { object-fit: var(--crs-fit); width:100%; height:100%; }
    #${rid}:hover .carousel-arrow { opacity:1; }
    #${rid} .carousel-arrow { opacity:0; transition:opacity .3s ease; z-index: 10; }
    #${rid} [data-dots] { z-index: 10; }
  </style>

  ${slidesHtml}

  <button type="button" class="carousel-arrow absolute top-0 left-0 flex items-center justify-center h-full px-4 group focus:outline-none" data-prev aria-label="Previous slide">
  ...
  <button type="button" class="carousel-arrow absolute top-0 right-0 flex items-center justify-center h-full px-4 group focus:outline-none" data-next aria-label="Next slide">
    <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-gray-500/50 group-hover:bg-gray-700/80">
      <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
  ...
  <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2" data-dots>
    ${dotsHtml}
  </div>
